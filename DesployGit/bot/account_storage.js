...
export class AccountStorageService {
  ...
  constructor(customConfig = {}) {
    ...
    // Tự động tìm vị trí output.txt phù hợp
    this._localOutputPath = path.join(__dirname, "output.txt");
    this._fallbackOutputPath = path.join(__dirname, "fallback_accounts.txt");
    this._secretsOutputPath = path.join(__dirname, "github-2fa-secrets.txt");
  }
  ...
  async _saveToLocalFiles({ email, username, password, twoFactorSecret, recoveryCodes, proxyStr, lineFormatted }) {
    try {
      ...
      console.log(`📁 [Storage Local] Đã lưu tài khoản vào: ${this._localOutputPath}`);
    } catch (err) {
  ...
  async _saveToFallbackFile(lineFormatted) {
    try {
      ...
      console.log(`💾 [Storage Fallback] Đã bảo vệ an toàn tài khoản tại: ${this._fallbackOutputPath}`);
    } catch (err) {
