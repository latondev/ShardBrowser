...
export class GmailCreatorClient {
  ...
  // Đọc toàn bộ API keys từ rapidapikey.md (tìm kiếm linh hoạt nhiều vị trí)
  _loadApiKeys() {
    const keys = [];
    const candidatePaths = [
      path.join(process.cwd(), "rapidapikey.md"),
      path.join(process.cwd(), "bot", "rapidapikey.md"),
      path.join(process.cwd(), "DesployGit", "bot", "rapidapikey.md"),
      path.join(process.cwd(), "Testing", "git", "rapidapikey.md"),
      path.join(__dirname, "rapidapikey.md")
    ];
  ...
  async createAccount(maxRetries = 2) {
    ...
    const quotaErr = new Error(`RAPIDAPI_QUOTA_EXHAUSTED: Toàn bộ ${this._apiKeys.length} key trong pool đều đã hết lượt tạo trong giờ này.`);
