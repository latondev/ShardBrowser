...
$testOAuthList = @(
    "https://github.com/login/oauth/authorize?client_id=327a3c3e2182283ccb8e&scope=user:email", # Supabase
    "https://github.com/login/oauth/authorize?client_id=01ab8ac9400c4e429b23&scope=user:email", # VS Code
    "https://github.com/login/oauth/authorize?client_id=40c5e73ef5e00fb3a7cc&scope=user:email", # Gitpod
    "https://github.com/login/oauth/authorize?client_id=1d18ca53ecbebbceb9c4&scope=user:email"  # Replicate
)

foreach ($url in $testOAuthList) {
    Write-Host "`nTest URL: $url"
    $res = $client.GetAsync($url).Result
    $finalUri = $res.RequestMessage.RequestUri.ToString()
    $html = $res.Content.ReadAsStringAsync().Result
    Write-Host "Final URI: $finalUri"

    if ($finalUri -match "/dashboard" -or $html -match "This account is flagged" -or $html -match "cannot authorize a third party application") {
        Write-Host "🔥 BẮT ĐƯỢC CỜ FLAGGED! (Redirect về dashboard hoặc chứa thông báo)" -ForegroundColor Red
        Write-Host "HTML match: $($html -match 'This account is flagged')"
    } else {
        Write-Host "Không redirect về dashboard. Trạng thái: Bình thường (Authorize page)" -ForegroundColor Green
    }
}
