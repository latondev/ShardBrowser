...
// ==============================================================================
async function main() {
  const totp = new TotpClient();
  const testSecret = process.argv[2] || "JBSWY3DPEHPK3PXP"; // Secret test chuẩn

  console.log("==================================================================");
  console.log("             TOTP CLIENT STANDALONE VERIFICATION                  ");
  console.log("==================================================================");
  console.log(`🔐 Secret Key       : ${testSecret}`);

  const t0 = Date.now();
  const code = totp.generateCode(testSecret);
  const duration = Date.now() - t0;
  const remaining = totp.getRemainingSeconds();

  console.log(`🔑 Mã TOTP 6 Số     : [ ${code} ]`);
  console.log(`⏱️ Thời gian còn lại: ${remaining} giây`);
  console.log(`⚡ Tốc độ sinh mã   : ${duration} ms (Offline)`);
  console.log("==================================================================");
}

if (process.argv[1] && (import.meta.url === `file://${process.argv[1].replace(/\\/g, "/")}` || process.argv[1].endsWith("totp_client.js"))) {
  main();
}
